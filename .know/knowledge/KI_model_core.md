...
- **Dual-Branch Projection (Balance)**: `FeatureProjector` использует две ветки: MLP (`proj`) и Residual (`residual_proj`). Каждая имеет свой обучаемый коэффициент масштабирования (`output_scale` и `residual_scale`). При обучении применяется `balance_loss` для выравнивания вклада обеих веток.
- **XLA FSDP Constraint**: Коэффициенты масштабирования должны быть как минимум 1D тензорами (shape=[1]), так как 0-dim тензоры ломают логику шардинга в XLA.
...
- **AWAKENED Initialization**: Чекпоинты "пробуждения" (Awakened) могут быть в формате FSDP (плоские ключи с `_orig_module.`). Система автоматически определяет формат и маппит веса на `ReasoningDistiller`.
