...
def train():
    ...
    criterion = DistillationLoss(cos_weight=0.0, lambda_norm=0.0)  # ADR-018: отказ от косинуса, ADR-024: отказ от Norm Correlation
    ...
    # Данные
    train_loader = get_dataloader(stage='reasoning', batch_size=2, max_length=2048, split='train', val_ratio=0.0)
    val_loader = get_dataloader(stage='reasoning', batch_size=2, max_length=2048, split='val', val_ratio=0.0)
    accumulation_steps = 8

    if rank == 0:
        wandb_kwargs = {
            "project": "BEBLaDII",
            "name": f"tpu-v4-spmd-resumed-{global_step}"
        }
        try:
            import wandb
