...
```
2. **Подключение**: добаьте флаг `-A` (проброс агента):
```powershell
gcloud compute tpus tpu-vm ssh bebladii-v4-8 --zone us-central2-b -- -A -L 8888:localhost:8888
```
...
```bash
# 1. Подготовка системы и создание venv
