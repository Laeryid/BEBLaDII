...
from src.beb_la_dii.utils.loss import safe_normalize, safe_cosine_similarity
...
class BEBLaDIIVAE(nn.Module):
    def __init__(self, qwen_model_path, vae_encoder, vae_decoder):
        super().__init__()
        self.qwen = AutoModelForCausalLM.from_pretrained(qwen_model_path, torch_dtype=torch.bfloat16)
    ...
    def forward(self, input_ids, attention_mask=None):
        ...
        outputs = self.qwen(inputs_embeds=projected_embeds, attention_mask=attention_mask)
        logits = outputs.logits
        
        return logits, z, mu, logvar, inputs_embeds


def compute_phase1_loss(logits, labels, z, mu, logvar, attention_mask, kl_beta=0.01, contrastive_lambda=0.01):
    metrics = {}
    ...
    ce_loss_raw = loss_fct(shift_logits.view(-1, shift_logits.size(-1)), shift_labels.view(-1))
    ...
    cov_loss = 2.0 * torch.where(_cov_abs < 1.0, 0.5 * cov_off_diag.pow(2), _cov_abs - 0.5).sum() / D
    metrics['cov_loss'] = cov_loss.detach()
    
    _v_diff = v_state - 1.0
    _v_abs = _v_diff.abs()
    prior_loss = m_state.pow(2).mean() + 2.0 * torch.where(_v_abs < 1.0, 0.5*_v_diff.pow(2), _v_abs - 0.5).mean() + 0.3 * cov_loss
...
def get_dataloader(data_path, batch_size=8, max_length=1024):
    if data_path.startswith("gs://"):
        ...
        files = fs.glob(data_path.rstrip('/') + "/*.parquet")
        data_files = ["gs://" + f if not f.startswith("gs://") else f for f in files]
    else:
        import glob
        import os
    ...
    print(f"Loading {len(data_files)} parquet files from {data_path}...")
...
def train(args):
    ...
    dataloader = get_dataloader(args.data_path, batch_size=args.batch_size, max_length=args.max_length)
    ...
    for epoch in range(args.epochs):
        for batch in dataloader:
            ...
            logits, z, mu, logvar, teacher_embeds = model(input_ids, attention_mask=attention_mask)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_path", type=str, default="Qwen/Qwen2.5-1.5B", help="Path to frozen Qwen 1.5B")
    parser.add_argument("--data_path", type=str, default="gs://bebladii-datasets-us/phase 3/train_data/data", help="Dataset path")
    parser.add_argument("--output_dir", type=str, default="checkpoints/phase1", help="Output directory")
    ...
    parser.add_argument("--wandb_project", type=str, default="BEBLaDII-Phase1", help="wandb project name (empty to disable)")
