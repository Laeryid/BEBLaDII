...
def compute_phase3_loss(
    ...
    # Вычисляем таргеты строго в float32, чтобы избежать потери точности
    denoise_target = safe_normalize(z_clean.float(), dim=-1)
    cos_denoise = (dus_final.float() * denoise_target).sum(dim=-1)
    ...
    # Заменяем c_true ** gamma на c_true, чтобы градиенты не обнулялись
    w_c = c_true.float()
    identity_target = safe_normalize(z_noisy.float(), dim=-1)
    cos_identity = (dus_final.float() * identity_target).sum(dim=-1)
