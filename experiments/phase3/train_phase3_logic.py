...
class BEBLaDIIPhase3(nn.Module):
    def __init__(
        ...
        self.qwen_embeddings = (
            _qwen_base.get_input_embeddings()
        )  # nn.Embedding (vocab, 1536)
        del _qwen_base
        ...
        if dus_weights and os.path.exists(dus_weights):
            ...
            print(
                f"[Init] Awakened DUS weights loaded from {dus_weights} (Matched {matched}/{total_keys} parameters)",
                flush=True,
            )
...
def compute_phase3_loss(
    ...
    metrics["norm_dus_delta"] = (
        dus_delta.float().norm(dim=-1) * noise_mask
    ).sum() / noised_tokens
...
def train(args):
    ...
    if os.path.exists(args.local_whitening):
        ...
        whitening_mean = None
        print(
            "[Init] WARN: Whitening matrix NOT found, using raw teacher vectors.",
            flush=True,
        )
    ...
    model.encoder = SpmdFullyShardedDataParallel(
        model.encoder, mesh=mesh, shard_output=shard_output
    )
    model.dus = SpmdFullyShardedDataParallel(
        model.dus, mesh=mesh, shard_output=shard_output
    )
    model.confidence_proj = SpmdFullyShardedDataParallel(
        model.confidence_proj, mesh=mesh, shard_output=shard_output
    )
    ...
    pbar = (
        tqdm(total=total_steps, desc="Training Phase 3")
        ...
        for batch in dataloader:
            ...
            if val_dataloader is not None and step > 0 and step % args.val_steps == 0:
                ...
                if num_v_batches > 0:
                    ...
                    if xm.is_master_ordinal():
                        if args.wandb_project:
                            wandb.log({"val/loss": v_loss_avg}, step=step)
            ...
            if step > 0 and step % args.save_steps == 0:
                ...
                if xm.is_master_ordinal():
                    ...
                    ema.apply(model)
                    dus_ema_state = {
                        k: v.cpu() for k, v in model.dus.state_dict().items()
                    }
                    proj_ema_state = {
                        k: v.cpu()
                        for k, v in model.confidence_proj.state_dict().items()
                    }
                    ema.restore(model)
                    ...
                    if args.gcs_checkpoint_dir:
                        ...
                        print(
                            f"[GCS] Syncing checkpoints to {args.gcs_checkpoint_dir}..."
                        )
                        subprocess.Popen(
                            [
                                "gcloud",
    ...
    if xm.is_master_ordinal():
        ...
        proj_ema_state = {
            k: v.cpu() for k, v in model.confidence_proj.state_dict().items()
        }
        ema.restore(model)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="BEBLaDII Phase 3 — DUS Logic Training"
    )
    ...
    parser.add_argument(
        "--denoise_delta", type=float, default=5.0, help="Delta для Huber Loss"
    )
        ...
        if not os.path.exists(args.local_encoder_weights):
            ...
            subprocess.run(
                [
                    ...
                    "storage",
                    "cp",
                    args.encoder_weights_gs,
                    args.local_encoder_weights,
                ],
                check=True,
            )
